...
async def main():
    ...
    key1 = os.getenv("OPENAI_KEY_1", "sk-example-key-1-not-a-real-key")
    key2 = os.getenv("OPENAI_KEY_2", "sk-example-key-2-not-a-real-key")
    ...
    try:
        ...
        print("  ✗ Budget exceeded (expected with hard enforcement)")
    ...
    try:
        response = await router.route(intent, objective="cost")
        print("  ✓ Request succeeded (soft enforcement allows it)")
        print(f"    Cost: ${response.cost.amount if response.cost else Decimal('0')}")
        print("    Note: Budget violation logged but request allowed")
    except BudgetExceededError as e:
    ...
    print("    Provider budget: Resets daily")
